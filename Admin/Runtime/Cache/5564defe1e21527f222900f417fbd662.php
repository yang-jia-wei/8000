...
<?php  $classify_id=pg('classify_id'); $recursive_classify_id=recursive_classify_id($classify_id,3)==''?1:recursive_classify_id($classify_id,3); ?>
<div class="content">
  ...
  <div class="right_panel" id="right">
	...
	<?php }?>
         ...
         <?php
 $classify = M('classify')->field('classify_id,level_id,classify_pid,type_id,classify_name,date,is_delete,classify_img,note,page_img')->where(array('classify_id'=>$recursive_classify_id,'level_id'=>array('gt',1)))->order('date asc')->select(); foreach($classify as $k=>$v){ ?>
    ...
    <?php  $arr=array('8888'); $zi=M('classify')->where(array('classify_pid'=>$v['classify_id']))->find(); if($v['level_id']==3 && $zi){ ?>
          ...
          <td><?php
$type = M('classify_type')->where(array('type_id'=>$v['type_id']))->find(); echo $type['type_name']; ?></td>
	...
	<?php $user=session('user'); if($user['user_id']==1){?>
          ...
          <select name="data[is_delete][]">
              <?php
 $input_p = M('input')->where(array('input_pid'=>351))->select(); foreach($input_p as $k2=>$v2){ ?>
            ...
            <?php }?>
          <td><?php $classify = M('classify')->where(array('classify_pid'=>$v['classify_id']))->find(); if(empty($classify)&&$count==0&&$v['is_delete']!=2){?>
	...
	<?php }?>
       ...
       <?php
global $list; $list = M('classify')->field('classify_id,level_id,classify_pid,type_id,classify_name,date,is_delete,classify_img,note,page_img')->where(array('version_id'=>$version_id))->order('date asc')->select(); function recursive_classify($classify_pid='1') { global $list; foreach($list as $k=>$v) { if($v['classify_pid']==$classify_pid) { ?>
        <tr class="row_list cid_<?php echo $v['classify_pid']; ?>" >
          <td height="30">&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $v['classify_id']?>
            <input name="classify_id[]" type="hidden" value="<?php echo $v['classify_id']?>" /></td>
          <td><?php for($i=3;$i<=$v['level_id'];$i++)echo "&nbsp;&nbsp;&nbsp;";?>
...
$type = M('classify_type')->where(array('type_id'=>$v['type_id']))->find(); echo $type['type_name']; ?></td>
 ...
 $input_p = M('input')->where(array('input_pid'=>351))->select(); foreach($input_p as $k2=>$v2){ ?>
            ...
            <?php }?>
          <td><?php $classify = M('classify')->where(array('classify_pid'=>$v['classify_id']))->find(); if(empty($classify)&&$count==0&&$v['is_delete']!=353){?>
            <a  href="javascript:;" onclick="if(confirm('确定删除吗!')){ajax_list('admin.php?m=classify&a=del_save&classify_id=<?php echo $v['classify_id'];?>','admin.php?m=classify&a=index&admin_classify_id=3')}" class="delete">删除</a>
            <?php }?></td>
        </tr>
        <?php
 recursive_classify($v['classify_id']); } } } recursive_classify($recursive_classify_id); ?>
      </table>
