...
<?php
global $lists,$classify_id; $classify_id=pg('classify_id'); $classify = M('classify')->where(array('classify_id'=>$classify_id))->find(); ?>
...
<select name="data[classify_pid]" id="classify_pid">
  ...
  <?php  $lists = M('classify')->field('classify_id,level_id,classify_pid,type_id,classify_name')->where(array('version_id'=>session('version_id')))->select(); function recursive_classify($classify_pid='1') { global $lists,$classify_id; foreach($lists as $k=>$v) { if($v['classify_pid']==$classify_pid) { ?>
...
<?php
 recursive_classify($v['classify_id']); } } } recursive_classify(session('version_classify_id')); ?>
</select>
</td>
  </tr>
...
<?php  $list = M('classify_type')->where(array('show_id'=>2))->order('date asc')->select(); foreach($list as $k=>$v){ ?>
  <option value="<?php echo $v['type_id'];?>"<?php if($classify['type_id']==$v['type_id']){?> selected="selected"<?php }?>><?php echo $v['type_name']?></option>
<?php }?>
</select>
</td>
  </tr>
<?php  $input = M('input')->where(array('type_id'=>4,'edit_switch'=>2,'input_pid'=>array('exp','is null')))->order('date asc')->select(); foreach($input as $k=>$v){ ?>
  <tr class="row">
