...
<?php
global $lists,$classify_pids,$classify_id; $classify_id=pg('classify_id'); $classify = M('classify')->where(array('classify_id'=>$classify_id))->find(); $lists = M('classify')->field('classify_id,level_id,classify_pid,type_id,classify_name')->where(array('version_id'=>session('version_id')))->select(); $classify_pids=$classify['classify_pid']; ?>
...
<?php
function recursive_classify($classify_pid='1') { global $lists,$classify_pids,$classify_id; foreach($lists as $key=>$val) { if($val['classify_pid']==$classify_pid) { ?>
  <option value="<?php echo $val['classify_id']?>"<?php if($classify_id==$val['classify_id']){?> disabled="disabled"<?php } if($classify_pids==$val['classify_id']){?> selected="selected"<?php }?>>┣<?php for($i=3;$i<=$val['level_id'];$i++)echo "━"; echo $val['classify_name']?></option>
<?php
 recursive_classify($val['classify_id']); } } } recursive_classify(session('version_classify_id')); ?>
</select>
...
<?php
$list = M('classify_type')->where(array('show_id'=>2))->order('date asc')->select(); foreach($list as $k=>$val){ ?>
...
<tr class="row">
 ...
 break; case 2: ?>
        ...
        <?php
 break; case 3: echo fckeditor('data['.$v['field_name'].']',$v['edit_size'],$classify[$v['field_name']]); break; case 4: ?>
        <?php
 $input_p = M('input')->where(array('input_pid'=>$v['input_id']))->select(); $valarr=unserialize($classify[$v['field_name']]); foreach($input_p as $k2=>$v2){ ?>
        <label>
 ...
 break; case 7: ?>
        ...
        <?php
 if($classify[$v['field_name']]!='') { echo '<a href="'.$classify[$v['field_name']].'" target="_blank"><img src="'.$classify[$v['field_name']].'" width="50" height="25"/></a>'; ?>
