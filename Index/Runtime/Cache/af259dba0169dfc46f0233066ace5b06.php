...
<!--首页中间（凡斯环保）-->
    <section class="fshb">
        ...
        <ul class="fshb-list flex-sb">
            <?php $goods=M()->table('index_goods n,index_relevance r')->where('r.classify_id =222 and r.content_id=n.goods_id')->order('date desc')->select(); foreach($goods as $k=>$v){ ?>
        ...
        <div class="container flex-sb">
            <div class="solutions-right">
                ...
                <ul class="solutions-ul">
                    <?php $goods2=M()->table('index_goods n,index_relevance r')->where('r.classify_id =224 and r.content_id=n.goods_id')->order('date desc')->select(); foreach($goods2 as $k2=>$v2){ ?>
        ...
        <div class="container">
            ...
            <div class="products-con">
                <ul class="products-tips flex-ce wow fadeInUp" data-wow-delay="0.1s">
                    <?php $goods3=M()->table('index_goods n,index_relevance r')->where('r.classify_id =225 and r.content_id=n.goods_id')->order('date desc')->select(); foreach($goods3 as $k=>$v){ ?>
                    <li><?php echo $v['goods_name'];?></li>
                    <?php }?>
                </ul>
                <div class="products-baogao flex-ce">
                    <?php $goods4=M()->table('index_goods n,index_relevance r')->where('r.classify_id =226 and r.content_id=n.goods_id')->order('date desc')->select(); foreach($goods4 as $k=>$v){ ?>
...
<!--服务案例-->
    <section class="service-case">
        <div class="container">
            ...
            <ul class="service-case-nav flex-ce wow fadeInUp" data-wow-delay="0.1s">
                <?php $goods5=M()->table('index_goods n,index_relevance r')->where('r.classify_id =227 and r.content_id=n.goods_id')->order('date desc')->select(); foreach($goods5 as $k=>$v){ ?>
            ...
            <div id="caselist" class="owl-carousel wow fadeInUp owl-theme" data-wow-delay="0.2s" style="opacity: 1; display: block;">
                <div class="owl-wrapper-outer"><div class="owl-wrapper" style="width: 7380px; left: 0px; display: block; transition: all 800ms ease 0s; transform: translate3d(-2460px, 0px, 0px);">
                        <?php $goods6=M()->table('index_goods n,index_relevance r')->where('r.classify_id =227 and r.content_id=n.goods_id')->order('date desc')->select(); foreach($goods6 as $k=>$v){?>
            ...
            <ul class="client-list flex-fs wow fadeInUp" data-wow-delay="0.3s">
                <?php $link=M()->table('index_link n,index_relevance r')->where('r.classify_id =229 and r.content_id=n.link_id')->order('date desc')->select(); foreach($link as $k=>$v){ ?>
    ...
    <section class="zl">
        ...
        <div class="zllist">
            <ul>
                <?php $news=M()->table('index_news n,index_relevance r')->where('r.classify_id =230 and r.content_id=n.news_id')->order('date desc')->select(); foreach($news as $k=>$v){ ?>
...
<!--        空气净化器-->
        <div class="prolist">
            ...
            <ul class="pro-cont flex-fs">
                <?php $goods=M()->table('index_goods n,index_relevance r')->where('r.classify_id =231 and r.content_id=n.goods_id')->order('date desc')->select(); foreach($goods as $k=>$v){ ?>
    ...
    <section class="news">
        <div class="container">
            ...
            <ul class="news-nav flex-ce wow fadeInUp" data-wow-delay="0.1s">
                <?php $list=M('classify')->where(array('classify_pid'=>209))->order('date asc')->select(); foreach($list as $k=>$v){ ?>
