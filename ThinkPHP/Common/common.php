...
*/
	$editorid='editor'.mt_rand(100000,999999);
	echo '<script type="text/javascript">KindEditor.ready(function(K){window.editor = K.create(\'#'.$editorid.'\');});</script>';
	echo '<textarea id="'.$editorid.'" name="'.$name.'" style="width:100%;height:'.$height.'px;">'.$value.'</textarea>';
}
...
function classify_url($type_id,$classify_id){
	...
	if(REWRITE==TRUE)
	{
		$url=$list['table_name'].'-'.$list['page_name'].'-'.$classify_id.'.html';
		if($list['table_name']=='')$url='index.html';
	}
	else
	{
		$url=__APP__.'?m='.$list['table_name'].'&a='.$list['page_name'].'&classify_id='.$classify_id;
		if($list['table_name']=='')$url='index.php';
	}
	if(trim($classify['classify_url'])!='')$url=$classify['classify_url'];
	return $url;
}
//内容地址
function content_url($type_id,$content_id){
	...
	if(REWRITE==TRUE)
	{
		$url=$list['table_name'].'-details-'.$content_id.'.html';
	}
	else
	{
		$url=__APP__.'?m='.$list['table_name'].'&a=details&content_id='.$content_id;
	}
	return $url;
}
/**
...
function fuwei()
{
	$fuwei=md5($_SERVER['SERVER_NAME']);
	$fuwei.=md5(substr($fuwei,0,1));
	$fuwei.=md5(substr($fuwei,8,9));
	$fuwei.=md5(substr($fuwei,8,9));
	$fuwei.=md5(substr($fuwei,2,3));
	$fuwei.=md5(substr($fuwei,7,8));
	$fuwei.=md5(substr($fuwei,1,2));
	$fuwei.=md5(substr($fuwei,3,4));
	return $fuwei;
}
...
function get_price($goods_id,$attr,$table='goods')
{
	$price_array=unserialize(M($table)->where(array($table.'_id'=>$goods_id))->getfield('price_array'));
...
function link_qrcode($order_id){
    ...
    $input->SetBody('订单号：'.$order_list['order_number'].'支付');
    ...
    $input->SetNotify_url("http://".$_SERVER['HTTP_HOST'].__ROOT__."/index.php/wxpay/payact");
    ...
    QRcode::png($link_url,$dir.'/'.$order_list['order_number'].'.png','M',5.7,3);
	$order_list['qrcode']=$dir.'/'.$order_list['order_number'].'.png';
    return $order_list['qrcode'];
}

function qrcode($value,$src,$matrixPointSize)//生成二维码
{
	include_once 'ThinkPHP/Extend/Vendor/phpqrcode/phpqrcode.php';
	$errorCorrectionLevel = 'L';//容错级别 
	//生成二维码图片  
	QRcode::png($value, 'qrcode.png', $errorCorrectionLevel, $matrixPointSize, 2);  
	//$logo = 'cn/images/qr_logo.png';//准备好的logo图片  
...
function get_site($version_id=1)
{
	$classify_id=pg('classify_id');
	$content_id=pg('content_id');
	$type_id=get_type_id();
	$table_name = M('classify_type')->where(array('type_id' => $type_id))->getField('table_name');
	$site = M('site')->where(array('version_id'=>$version_id))->find();
	$seo=M('seo')->where(array('module_name'=>MODULE_NAME,'action_name'=>ACTION_NAME,'version_id'=>$version_id))->find();
	if(!empty($seo))
	{
		$site['title']=$seo['title'];
	...
	if($content_id)
	{
		...
		if($type_id==3)
		{
			$site['title']=$content['title']==''?$content['goods_name']:$content['title'];
			$site['keywords']=$content['keywords']==''?$content['goods_name']:$content['keywords'];
			$site['description']=$content['description']==''?$content['goods_name']:$content['description'];
		}
		else if($type_id==14)
		{
			$site['title']=$content['title']==''?$content['news_title']:$content['title'];
			$site['keywords']=$content['keywords']==''?$content['news_title']:$content['keywords'];
			$site['description']=$content['description']==''?$content['news_title']:$content['description'];
		}
	}
	else if($classify_id)
	{
		$classify = M('classify')->where(array('classify_id' => $classify_id))->find();	
		$site['title']=$classify['title']==''?$classify['classify_name']:$classify['title'];
		$site['keywords']=$classify['keywords']==''?$classify['classify_name']:$classify['keywords'];
		$site['description']=$classify['description']==''?$classify['classify_name']:$classify['description'];
	}
	else
	{
		$classify = M('classify')->where(array('classify_id' => 3))->find();	
		$site['title']=$classify['title']==''?$classify['classify_name']:$classify['title'];
		$site['keywords']=$classify['keywords']==''?$classify['classify_name']:$classify['keywords'];
		$site['description']=$classify['description']==''?$classify['classify_name']:$classify['description'];
	}
	$site['title']=$site['title'].'_'.$site['company_name'];
	$site['keywords']=$site['keywords'].'_'.$site['company_name'];
	$site['description']=$site['description'].'_'.$site['company_name'];
	return $site;
}

global $classify_id_arr;
function recursive_child_classify($classify_pid,$level=1,$current_level=1)
{
	global $classify_id_arr;
	$list = M('classify')->field('classify_id')->where(array('classify_pid'=>$classify_pid))->order('date asc')->select();
	$current_level++;
	foreach($list as $k=>$v)
	{
		$classify_id_arr[]=$v['classify_id'];
		$classify = M('classify')->field('classify_id')->where(array('classify_pid'=>$v['classify_id']))->order('date asc')->select();
		if(!empty($classify) && $level>=$current_level)
		{
			recursive_child_classify($v['classify_id'],$level,$current_level);
		}
	}
}
function get_child_classify($classify_id,$level=1,$type_id=3)
{
	global $classify_id_arr;
	recursive_child_classify($classify_id,$level);
	return $classify_id_arr;
}
