...
class  ThinkTemplate {
    ...
    protected function compiler($tmplContent) {
        ...
        if(C('TMPL_STRIP_SPACE')) {
            ...
            $tmplContent    = preg_replace_callback($find, function($r)use($replace){return $replace;}, $tmplContent);
        }
        // 优化生成的php代码
    ...
    protected function parsePhp($content) {
        if(ini_get('short_open_tag')){
            ...
            $content = preg_replace_callback('/(<\?(?!php|=|$))/i', function($r){return $r[1]."\n";}, $content );
        }
        // PHP语法检查
    ...
    protected function parseExtend($content) {
        ...
        if($find) {
            ...
            preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is',function ($r){return $this->parseBlock($r[1], $r[2]);},$content);
            // 读取继承模板
            $array      =   $this->parseXmlAttrs($matches[1]);
            ...
            $content    =   preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is',function($r){return stripslashes($r[2]);},$content);
        }
        return $content;
    }
        ...
        foreach ($tLib->getTags() as $name=>$val){
            ...
            foreach ($tags as $tag){
                ...
                if (!$closeTag){
                    ...
                    for($i=0;$i<$level;$i++) 
                        //$content=preg_replace($patterns,$replacement,$content);
                        $content=preg_replace_callback($patterns,function($r)use($tagLib, $tag){return $this->parseXmlTag($tagLib, $tag, $r[2]);},$content);
                }
            }
        }
    }
